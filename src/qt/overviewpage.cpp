...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizenihiloAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizenihiloAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizenihiloAmount = strAnonymizenihiloAmount.remove(strAnonymizenihiloAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizenihiloAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizenihiloAmount*COIN) nMaxToAnonymize = nAnonymizenihiloAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizenihiloAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizenihiloAmount));
        strAnonymizenihiloAmount = strAnonymizenihiloAmount.remove(strAnonymizenihiloAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizenihiloAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
