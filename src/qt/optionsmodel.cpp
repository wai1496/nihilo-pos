...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizenihiloAmount"))
        settings.setValue("nAnonymizenihiloAmount", 1000);
    nAnonymizenihiloAmount = settings.value("nAnonymizenihiloAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizenihiloAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizenihiloAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizenihiloAmount:
            return QVariant(nAnonymizenihiloAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizenihiloAmount:
            nAnonymizenihiloAmount = value.toInt();
            settings.setValue("nAnonymizenihiloAmount", nAnonymizenihiloAmount);
            emit AnonymizenihiloAmountChanged(nAnonymizenihiloAmount);
            break;
        default:
            break;
        }
