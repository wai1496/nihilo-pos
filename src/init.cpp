...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: nihilo.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: nihilod.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizenihiloamount=<n> " + _("Keep N Nihilo anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Nihilo is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nihilo is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Nihilo version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Nihilo server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nihilo") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Nihilo to complete") << "\n";
    ...
    nAnonymizenihiloAmount = GetArg("-anonymizenihiloamount", 0);
    if(nAnonymizenihiloAmount > 999999) nAnonymizenihiloAmount = 999999;
    if(nAnonymizenihiloAmount < 2) nAnonymizenihiloAmount = 2;
    ...
    LogPrintf("Anonymize Nihilo Amount %d\n", nAnonymizenihiloAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1NIHL+1000 == (.1NIHL+100)*10
       10NIHL+10000 == (1NIHL+1000)*10
    */
